    ...
    for (int i = 0; i < particles.size() - 1; ++i) {
        for (int j = i + 1; j < particles.size(); ++j) {
            ...
            if (r < cutoff && r > minDistance) {
                ...
                // Splitting the force into its respective components and appending them to the two particles
                ...
                // append the potential enrgy due to hte two particles to the Total potential energy
...
int main() {
    ...
    int numSteps = 15000000;             // !Number of simulation steps
    ...
    int NoOfPeriods=80000;               // No of time velocity intialization is to be applied
    double minSeparation = 1.0;          // Minimum separation between particles (adjust as needed)

    //Calculate and print the packing fraction
    ...
    // Intialize a list of n particles
    vector<Particle> particles(numParticles);
    // Empty velocity vector to be used in velocity initialization
    vector<double> velocities;
    // Initialize the system by placing particles at repective possitions
    ...
    for (int step = 0; step < numSteps; ++step) {
        ...
        // Calculate and store the total kinetic energy per particle for every nth data (to reduce the ammount of data points)
        ...
        //Snippet to print code completion percentage
