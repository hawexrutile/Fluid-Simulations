...
void calculateForces(std::vector<Particle>& particles, double L) {
    ...
    for (int i = 0; i < particles.size() - 1; ++i) {
        for (int j = i + 1; j < particles.size(); ++j) {
            ...
            double minDistance = 0.8 * sigma; // Minimum distance to prevent particle overlap

            if (r < cutoff && r > minDistance) {
                double force = 24.0 * epsilon * (2.0 * std::pow(sigma, 12.0) / std::pow(r, 13.0) - std::pow(sigma, 6.0) / std::pow(r, 7.0));
