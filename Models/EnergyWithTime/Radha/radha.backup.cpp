double updatePositionsAndVelocities(vector<Particle>& particles, double dt, double L,double sigma, double epsilon, double temperature, double gamma) {
    ...
    for (auto& particle : particles) {
        ...
        double v_x = particle.vx; double v_y = particle.vy; double v_z = particle.vz;
        double a_x = particle.ax; double a_y = particle.ay; double a_z = particle.az;

        Ax = 0.5 * dt *dt * (a_x - gamma * v_x) + var * sqrt(dt) * dt * (0.5 * noise_xi + (0.5/sqrt(3)) * noise_eta) ;
	    Ay = 0.5 * dt *dt * (a_y - gamma * v_y) + var * sqrt(dt) * dt * (0.5 * noise_xi + (0.5/sqrt(3)) * noise_eta) ;
	    Az = 0.5 * dt *dt * (a_z - gamma * v_z) + var * sqrt(dt) * dt * (0.5 * noise_xi + (0.5/sqrt(3)) * noise_eta) ;


        // Update positions
        particle.x += particle.vx * dt + Ax;
        particle.y += particle.vy * dt + Ay;
        particle.z += particle.vz * dt + Az;
    ...
    for (auto& particle : particles) {
        ...
        // Update velocities (half step)
        particle.vx += 0.5 * dt * (a_x + a_xn) - dt * gamma * particle.vx + var * sqrt(dt) * noise_xi - gamma * Ax;
        particle.vy += 0.5 * dt * (a_y + a_yn) - dt * gamma * particle.vy + var * sqrt(dt) * noise_xi - gamma * Ay;
        particle.vz += 0.5 * dt * (a_z + a_zn) - dt * gamma * particle.vz + var * sqrt(dt) * noise_xi - gamma * Az;
    }
...
void initializeSystem(vector<Particle>& particles, int numParticles, double boxSize) {
    ...
    double spacing = 1.0; //boxSize/double(particlesPerDimension);
...
int main() {
    ...
    string comment="just checking if the pf in IS() was the actual issue all along gamma=1.0";
