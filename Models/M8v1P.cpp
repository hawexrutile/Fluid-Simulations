...
void updatePositionsAndVelocities(vector<Particle>& particles, double L, double dt) {
    ...
    for (auto& particle : particles) {
        ...
        if (vNc[i] == 0.0) {dphi_i=Lambda_i(gen) * sqrt(dt)*0.5;} // Avoid division by zero
        else {dphi_i = (Omega / vNc[i]) * vsum_term[i] * half_dt + Lambda_i(gen)* sqrt(dt)*0.5;} // Calculate angle to update phi_i with i
        particle.phi += dphi_i;
        i+=1;
